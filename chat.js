...
(function () {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const assistantText = (data && (data.reply || data.output_text)) ? (data.reply || data.output_text) : 'Unexpected response format from server.';
  ...
  // If hymAuth is available, listen for possible external session changes by polling.
  if (window.hymAuth) {
    setInterval(refreshInputState, 5000);
  }
})();
