...
//
// Tracks per-user usage counters for messages, image generation, and video
// generation. Counters are stored in Redis and reset automatically every 24 hours.
//
 ...
 */
const PLAN_LIMITS = {
  starter: { messages_per_day: 30,  images_per_day: 5,   videos_per_day: 0   },
  basic:   { messages_per_day: 150, images_per_day: 25,  videos_per_day: 0   },
  premium: { messages_per_day: 500, images_per_day: 100, videos_per_day: 10  },
  ultimate:{ messages_per_day: null, images_per_day: null, videos_per_day: null }
};
 ...
 *
 * @param {string} userId
 * @returns {Promise<{ messages_used: number, images_used: number, videos_used: number, last_reset: number }>}
 */
async function getUsage(userId) {
  const raw = await redis.command('GET', buildKey(userId));
  if (!raw) {
    return { messages_used: 0, images_used: 0, videos_used: 0, last_reset: Date.now() };
  }
  try {
 ...
 *
 * @param {string} userId
 * @param {{ messages_used: number, images_used: number, videos_used: number, last_reset: number }} data
 ...
 *
 * @param {{ messages_used: number, images_used: number, videos_used: number, last_reset: number }} data
 * @returns {{ messages_used: number, images_used: number, videos_used: number, last_reset: number }}
 */
 ...
 * @param {string} actionType - Action being tracked: 'message' | 'image' | 'video'
